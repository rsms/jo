...
var Env = Object.create(null, {
  ...
  JOROOT: {
    enumerable: true,                                  // {JOROOT}/bin/.jo/index.js
    value: process.env.JOROOT || (process.env.JOROOT = path.dirname(path.dirname(__dirname)))
  },
  ...
  open: {value:(filename, flags, mode) =>
    fsTryDirs1(filename, (path, cb) =>
      fs.openAsync(path, flags, mode, cb)) },

  // Like asyncfs.readdir, but JOROOT and JOPATH-aware.
  // async readdir(path:string):[ents:string[], pkgdir:string, jopath:string]
  readdir: {value:(dirname) => fsTryDirs1(dirname, fs.readdirAsync) },
