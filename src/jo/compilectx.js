...
// import {types as t} from 'npmjs.com/babel-core' [BUG] codegen broken
...
class CompileContext {
  constructor(pkg:Pkg, file:SrcFile, target:Target, log:Logger) {
  ...
  addImplicitImport(ref, specs, node=null) { // old: joAddImplicitImport
  ...
  registerExport(name, node, isImplicitExport=false) { // old: joRegisterExport
    var errmsg, existingExport = this.pkg.exports[name];
    ...
    if (name === 'default') {
      ...
      Object.keys(this.pkg.exports).forEach(k => {
        var exp = this.pkg.exports[k];
      ...
      let defaultExp = this.pkg.exports['default'];
      if (defaultExp) {
        if (isImplicitExport) {
  ...
  verifyReference(name, node, parent, scope, isNewName:bool) {
    ...
    //
    if (node.name in this.target.globals) {
      return;
    }

    // Resolves to something in this scope?
    let hasBinding = scope.hasBinding(node.name, /*noGlobals=*/true);
    if (hasBinding) {
      return;
    }
    ...
    if (!this.file.unresolvedIDs[node.name]) {
      this.file.unresolvedIDs[node.name] = {node:node};
      // console.log('verifyReference("'+name+'"): reg unresolvedIDs["'+node.name+'"]')
    }
    if (this.file.superclassReferences) {
      let superclassRef = this.file.superclassReferences[node.name];
      if (superclassRef) {
        if (!this.file.unresolvedSuperclassIDs) { this.file.unresolvedSuperclassIDs = {}; }
        this.file.unresolvedSuperclassIDs[node.name] = superclassRef;
        // console.log('verifyReference("'+name+'"): reg unresolvedSuperclassIDs["'+node.name+'"]')
      }
    }
  }
  ...
  _registerIDDefinitionInFile(scope, name, binding) {
    ...
    if (this.file._classDeclarationIDs[name]) {
    ...
    // console.log('_registerIDDefinitionInFile definedIDs["'+name+'"] = binding')
    // console.log('_registerIDDefinitionInFile this.file:', repr(this.file,0));
  }
  ...
  _registerIDDefinitionInPkg(scope, name, binding) {
    ...
    if (!this.pkg.definedIDs) {
      ...
      if (existing) {
        let newfound = IDBinding(binding.id, binding.kind, this.file)
    ...
    this.pkg.definedIDs[name] = {binding:binding, file:this.file};
    ...
    if (name === 'main' && binding.path.type === 'FunctionDeclaration') {
      ...
      this.pkg.mainFunc = {node: binding.node/*<-FIXME*/, file: this.file};
    }
  }
  ...
  registerIDDefinitions(scope) {
    ...
    Object.keys(scope.bindings).forEach(name => {
      ...
      switch (b.kind) {
        ...
        case 'unknown': {
          this.log.warn('registerIDDefinitions: unexpected binding', repr(b,1));
          break;
        }
